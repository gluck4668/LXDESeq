...
pack <- data.frame(c("BiocManager","openxlsx","dplyr","ggplotify"))

bioc_pack <- data.frame(c("DESeq2",'factoextra',"SummarizedExperiment","geneplotter","limma"))
...
table(duplicated(gene_set_df[,1])) # check the duplicated genes

gene_set_df <- limma::avereps(gene_set_df[,-1],ID= gene_set_df[,1]) %>% data.frame() # 对重复的gene_id,取其平均值，同时也有去重功能

gene_set_df$sum <- rowSums(gene_set_df)

gene_set_df <- dplyr::filter(gene_set_df,sum>0)

gene_set_df <- gene_set_df[,-ncol(gene_set_df)]

gene_df <- round(gene_set_df,0) # DESeq2 要求数据全部为整数，不能有小数
...
gg_clustering <- as.ggplot(clustering)

ggsave("analysis results/hierarchical clustering.png", gg_clustering,width=1200, height =900, dpi=150,units = "px")

# 利用PCA主成分分析的方法评估一下数据质量。

rld <- vst(dds, blind=FALSE)     #vst()函数效果和rlog（）一样，且速度更快。
PCA <- plotPCA(rld, intgroup="condition",ntop=500)

gg_pca <- as.ggplot(PCA)

ggsave("analysis results/PCA.png", gg_pca,width=900, height =900, dpi=150,units = "px")
...
write.xlsx (resdata, "analysis results/DESeq analysis result.xlsx")
